...
void recibirPCB(int socket,uint32_t tamanio) {
	...
	free(mensaje);
	setPCB(pcb);
	//Por ahora no se va utilizar esto
	//notificarAMemoriaElCambioDeProceso(pcb->PID);
			...
			if (strcmp(proximaInstruccion, "FINALIZAR") == 0) {
				...
				switch (operacion) {
					...
					if(pcb->quantum!=0)
					 ...
					 log_debug (ptrLog,"este es el quantum trabajado %d \n", pcb->quantum_sleep);
				//	 printf("este es el quantum trabajado %d \n", pcb->quantum_sleep);
			}

		}else{
			log_info(ptrLog, "No se pudo recibir la instruccion de UMC. Cierro la conexion");
			//finalizarConexion(socketMemoria);
			return;
