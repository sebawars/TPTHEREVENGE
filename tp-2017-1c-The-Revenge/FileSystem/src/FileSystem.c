...
void cleanBitArray(){
	...
	log_info(logFile,"se limpio bitArray");
}
...
void obtenerDatos(char*ruta, int16_t offset, int16_t size) {
	...
	if(validarArchivoSinEnvio(path)<0){
		...
		log_info(logFile,"Se obtuvo correctamente >%s< del archivo %s",buffer,ruta);
		resultado->resultado=EXITO;
...
int guardarDatos(char*ruta, int16_t offset, int16_t size, char* buffer) {
...
int leerDatosBin(int bin, int offset,uint16_t* size,char** buffer){
	...
	faltaLeer=cant-(tamanio-offset);
	FILE* archivo =fopen(path,"r");
	rewind(archivo);
	if(offset>0)
		fseek(archivo,offset,SEEK_SET);
	fgets(leido, cant+1, archivo);
	if (strlen(leido)==0)
		return-1;
...
int escribirDatosBin(int bin, int offset,int* size,char** buffer){
	char* path=binAPath(bin);
	char* bufferAux=*buffer;
	int resto=*size,disponible;
	disponible=tamanioArchivo(binAPath(bin));
	int espacio=metadata->TAM_BLOQUES-disponible;
