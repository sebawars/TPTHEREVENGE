  ...
  int inicializarPrograma (int socketMemoria , t_PCB * pcb, int stack_size)
{
	...
	//			pcb->EC=0;
				...
				pcb->tablaHeap->paginasLiberadas=list_create();
				...
				syscalls->cantAlocar=0;
				syscalls->cantLiberar=0;
				syscalls->cantBorrar=0;
				syscalls->cantSignal=0;
				syscalls->cantWait=0;
				syscalls->cantMoverCursor=0;
	...
	if (nbytes > 0) {
		...
		switch(operacion)
		{
			case ABRIR_ARCHIVO:
				...
				{
					char *mensaje=uint32_serializer(&fd,&header.length);
					log_debug(logFile, "Se abre correctamente el archivo solicitado, FileDescriptor Asignado = %d con CPU, socket: %d",fd,socketCPU);
					header.type=SUCCESS;
		...
		case MOVER_CURSOR:
				...
				memcpy(&posicion,operacionDescriptorArchivo->informacion,operacionDescriptorArchivo->tamanio);
				if(moverCursor(operacionDescriptorArchivo->descriptor,operacionDescriptorArchivo->pid,posicion))
				{
					header.length=0;
					header.type=SUCCESS;
					sockets_send(socketCPU,&header,"\0");
		...
		case ESCRIBIR_ARCHIVO:
			...
			if (operacionDescriptorArchivo->descriptor == DESCRIPTOR_SALIDA) {
				...
				char *mensaje = datosPrograma_serializer(arch,&header.length);
	...
	if(nbytes>0)
	{
		...
		switch(operacion)
		{
				...
				if(semaforo!=NULL){
					if(semaforo->valor > 0){
						...
						if (pcb!=NULL)
						{
							...
							{
								return prog->PID == pcbBloqueado->pcb->PID;
							}
			...
			case SIGNAL:
				if(semaforo!=NULL)
				{
					log_debug(logFile, " [Semaforo] Se recibe Signal de CPU: %d de semaforo %s",
							socketCPU,semaforo->nombre);
					...
					if(semaforo->valor < 0)
					{
						...
						cantElement=queue_size(colaBlock);
						for(i=0;i<cantElement;i++)
						{
							//sem_wait(&mutexLista);
							t_pcbBloqueado* pcbBloqueado = queue_pop(colaBlock);
							//sem_post(&mutexLista);
							log_debug(logFile, " [Semaforo] pcbBloqueado->nombreSemaforo= %s -- semaforo->nombre = %s",
									pcbBloqueado->nombreSemaforo,semaforo->nombre);
							if(strcmp(pcbBloqueado->nombreSemaforo,semaforo->nombre)==0 && encontrado==false)
							{
								...
								//sem_wait(&mutex_lista_pcb);
								t_PCB* lPCB = list_find(listaPCB,(void*) _tieneElPid);
								t_cantSyscalls* sys = list_find(tablaSyscalls, (void*)_tieneElPid);
								sys->cantSignal++;
								//sem_post(&mutex_lista_pcb);
								...
								log_info(logFile, "El programa con PID= :%i ha salido de la cola de"
										"bloqueados para ingresar a la cola de listos",pcbBloqueado->pcb->PID);

								log_info(logFile, "El programa con PID:%i ha reingresado a la lista"
										"de PCB con sus atributos actualizados", pcbBloqueado-> pcb->PID);
