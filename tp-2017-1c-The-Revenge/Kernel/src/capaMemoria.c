...
int encontrarBloquesVecinosLibres(t_list* bloques, t_bloque* bloque,t_pagina_heap* pag)
{
	...
	if(indice!= -1)
	{
		...
		if(indice<cantBloques)
		{
			...
			if(aux!=-1){
				pag->tam_disponible=pag->tam_disponible+tamMetadata;
...
void liberarPagina(t_tabla_heap* tabla,t_pagina_heap* pagina)
{
	...
	if(list_is_empty(bloquesUsados))
	{
		...
		int* pagLibre=malloc(sizeof(int));
		*pagLibre=pagina->numero_pagina;

		list_add(tabla->paginasLiberadas,pagLibre);

		log_info(logFile, "[HEAP] Se ha liberado la pagina:%i de heap del programa con "
...
void operacionesHeap(int socketCPU, int tamanio, int operacion, int socketMemoria)
{
	...
	if(nbytes>0)
	{
		...
		*valorNumerico=datosHeap->puntero;
        ...
        t_cantPagsHeap* pagsHeap=list_find(tablaHeap, (void*)_tieneElPid);
        t_cantSyscalls* syscalls= list_find(tablaSyscalls, (void*)_tieneElPid);
		...
		switch(operacion){
			...
			if (!supera)
			{
				if (list_is_empty(pcb->tablaHeap->pagina))
				{
					...
					{
						offsetADevolver+=(pcb->primerPaginaStack + archC->STACK_SIZE)*tamanioPagina;
					...
					} else if(list_is_empty(pcb->tablaHeap->paginasLiberadas))
					{
						...
						//tengo una pagina que libere anteriormente, no pido otra pagina a memoria
						//la trato como si fuera la primera vez que la uso
						int numPag= (int)list_remove(pcb->tablaHeap->paginasLiberadas,0);
						...
						bloqueUsado->heapMetadata=heapOcupado;
						bloqueUsado->numeroBloque=1;
						bloqueUsado->offset = numPag*tamanioPagina + tamMetadata;
						bloqueUsado->data=malloc(bloqueUsado->heapMetadata->size);

						t_bloque* bloqueLibre = malloc(sizeof(t_bloque));
						bloqueLibre->heapMetadata = heapLibre;
						bloqueLibre->numeroBloque = 0;
						bloqueLibre->offset = (numPag + 1 )*tamanioPagina - bloqueLibre->heapMetadata->size;

						t_pagina_heap* exPagLibre= malloc(sizeof(t_pagina_heap));
						exPagLibre->numero_pagina= numPag;
						exPagLibre->tam_disponible= heapLibre->size;
						exPagLibre->bloques=list_create();

						list_add(exPagLibre->bloques, bloqueUsado);
						list_add(exPagLibre->bloques, bloqueLibre);

						list_add(pcb->tablaHeap->pagina,exPagLibre);

						offsetADevolver=bloqueUsado->offset;
						offsetADevolver+=(pcb->primerPaginaStack + archC->STACK_SIZE)*tamanioPagina;
						...
						logEstadoPaginaHeap(exPagLibre, pcb->PID);
			...
			case LIBERAR:
				...
				if(1)
				{
					syscalls->cantLiberar++;
				//Se ajusta la pagina al manejo de Heap - Se elimina la referencia respecto a las paginas de codigo y Stack
				int pagina = (*valorNumerico / tamanioPagina)-(pcb->primerPaginaStack+archC->STACK_SIZE);
...
//todo ver por que tira basura ese tam disponible
							pag->tam_disponible=pag->tam_disponible+bloque->heapMetadata->size;
							...
							sockets_send(socketCPU,&header,"\0");
							pagsHeap->liberarBytes=pagsHeap->liberarBytes + bloque->heapMetadata->size;
...
void almacenar(char* buffer,uint32_t valorNumerico,uint32_t pid){
	...
	log_debug(logFile, "guardando %s en la posicion %d por parte del proceso %d",buffer,valorNumerico,pid );
	//seteo pedido de escritura a memoria
	t_solicitudEscritura* mensaje= malloc(sizeof(t_solicitudEscritura));
	mensaje->pagina= pagina;
	mensaje->offset= offset;
	mensaje->tamanio=strlen(buffer);
	mensaje->pid=pid;
	...
	if(nbytes<=0)
	{
		log_info(logFile, "Error al mandar a guardar %s en la posicion %d por parte del proceso %d",buffer,valorNumerico,pid );
		free(serializer);
	}else{
			recv(socketMemoria, &header, sizeof(header_t), MSG_WAITALL);
			if(header.type==SUCCESS)
				log_debug(logFile, "se guardo correctamente %s en la posicion %d por parte del proceso %d",buffer,valorNumerico,pid );
			else
				log_error(logFile, "No %s en la posicion %d por parte del proceso %d",buffer,valorNumerico,pid );
