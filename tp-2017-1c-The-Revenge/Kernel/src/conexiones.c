...
void operacionPCB(int i, header_t header, uint32_t exitcode)
{
	...
	if(exitcode != FINALIZAR_CORRECTAMENTE && !nPCB->abortarEjecucion)
	{
		...
		log_info(logFile, "El programa con pid:%i sera finalizado Correctamente, el pcb no esta en la cola de aSerEliminados,:%i",ePCB->pcb->PID);
		...
		log_info(logFile, "El programa con pid:%i sera finalizado, entrara a la cola de aSerEliminados por accion de Consola (Kernel o Consola)",ePCB->pcb->PID);

		list_add(aSerEliminado, ePCB);
		/*
		if(ePCB->aviso==FINALIZACION_KERNEL)
		{
			ePCB->aviso=DEVOLUCION_CPU_KERNEL;
		}else if(ePCB->aviso==FINALIZACION_CONSOLA)
		{
			ePCB->aviso=DEVOLUCION_CPU_CONSOLA;
		}*/
	}
			...
			if (FD_ISSET(i, &setAux)) {
				...
				// Me fijo en el set de descriptores a ver cual respondió
					...
					case FINALIZAR_PROGRAMA:
						...
						sem_wait(&mutexLista);
						if(tieneElPidParaColas(colaNew, pid) != -1)
						{
							...
							log_info(logFile, "Se recibio una finalizacion abortiva de Consola, el proceso con pid:%i esta"
									"en la colaNew", nPCB->PID);
						}else if (list_any_satisfy(listaPCB,(void*)_tieneElPid)){
...
{
	int nbytes;
	//todo borrar printf para la entrega
