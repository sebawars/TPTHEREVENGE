...
void obtenerListado(int pid)
{
	...
	}else if (pid ==5)
	{
		...
		if(cant!=0)
		{
			...
			for (i = 0; i <cant ; i++)
			{
				t_pcbBloqueado *aux = queue_pop(auxcolaBlock);
				printf("-PID:%i ", aux->pcb->PID);
...
void operacionKernel(uint32_t operacion, int pid, int grado, uint32_t exitcode)
{
	...
	if(pcb!= NULL)
	{
		...
		printf("Borrar Archivos:%i veces.\n", syscall->cantBorrar);
		printf("Mover Cursor:%i veces.\n", syscall->cantMoverCursor);
		printf("Alocar:%i veces.\n", syscall->cantAlocar);
		printf("Liberar:%i veces.\n", syscall->cantLiberar);
		printf("Wait:%i veces.\n", syscall->cantWait);
		printf("Signal:%i veces.\n", syscall->cantSignal);
		...
		printf("Aloco %i bloques con un total de %i bytes de data y %i bytes de metadata.\n", pagsHeap->alocarCant, pagsHeap->alocarBytes, pagsHeap->alocarCant*(sizeof(int)+sizeof(bool)));
		printf("Libero %i bloques con un total de %i bytes de data %i bytes de metadata\n", pagsHeap->liberarCant,pagsHeap->liberarBytes, pagsHeap->liberarCant*(sizeof(int)+sizeof(bool)));
		...
		if (strcmp(&mensaje[0], "1") == 0)
		{
			...
			if (list_any_satisfy(listaPCB,(void*)_tieneElPid))
			{
				...
				if(lPCB != NULL && !lPCB->abortarEjecucion && lPCB->estado!=TERMINATED)
				{
					...
					lPCB->EC=KERNEL_FINALIZAR_PROGRAMA;
			...
			}else if(tieneElPidParaColas(colaNew, pid) != -1)
				{
					...
					t_PCB* nPCB=queue_get(colaNew,pid);
					nPCB->abortarEjecucion=true;
					nPCB->EC=KERNEL_FINALIZAR_PROGRAMA;
					queue_add_index(colaNew,nPCB,indice);
					printf("el proceso con pid:%i tiene EC:%i y abortarEj:%i el valor de kernel_finalizar es:%i\n",nPCB->PID, nPCB->EC, nPCB->abortarEjecucion, KERNEL_FINALIZAR_PROGRAMA);
